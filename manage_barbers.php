...
if (isset($_POST['add_barber'])) {
    ...
    if ($stmt->execute()) {
        header("Location: barbers.html?success=Barber added");
    } else {
        header("Location: barbers.html?error=" . urlencode($stmt->error));
    }
    exit();
}
...
if (isset($_POST['delete_barber'])) {
    ...
    if ($stmt->execute()) {
        header("Location: barbers.html?success=Barber removed");
    } else {
        header("Location: barbers.html?error=" . urlencode($stmt->error));
    }
    exit();
